"""Postgres store for the training diary's planned workouts and goals.

One row per planned item, scoped to one athlete like every other table here.
Plain dicts in and out — a planned item is five scalar columns, not a document,
so a domain dataclass here would be pure ceremony.
"""
...
_SELECT = "select id, kind, date, title, body, importance from planned_items"
...
class PostgresPlannedItemRepository:
    ...
    def list_range(self, start: date_type, end: date_type) -> List[Dict[str, Any]]:
        rows = self.db.fetch_all(
            f"{_SELECT} where athlete_id = %s and date >= %s and date <= %s "
            f"order by date",
            (self.athlete_id, start, end),
        )
    ...
    def create(
        ...
        self.db.execute(
            "insert into planned_items (id, athlete_id, kind, date, title, body, "
            "importance) values (%s, %s, %s, %s, %s, %s, %s)",
            (item_id, self.athlete_id, kind, date, title, body, importance),
        )
        return {
